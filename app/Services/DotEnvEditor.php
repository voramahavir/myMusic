    ...
    public function write($values = [])
    {
        ...
        foreach ($values as $key => $value) {
            ...
            if (str_contains($content, $key.'=')) {
                ...
                //env key has a value, replace it
                if ($matches[2]) {
                    $content = str_replace($matches[2], $value, $content);

                //env key has no value, replace "key=" with "key=value"
                } else {
                    $content = str_replace($matches[1], $matches[1].$value, $content);
                }
